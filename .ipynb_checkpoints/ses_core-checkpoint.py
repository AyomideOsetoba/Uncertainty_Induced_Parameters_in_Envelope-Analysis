...
def ses(signal, nfft, fs, bpfo_list, hilb, p_type, bw, p_spect=1, band=True):
    """
    ...
    p_spect: int
        The power applied to the spectrum
    bw: Int
        bandwith in Hz.
    band : bool, optional
    ...
    if hilb == 't':
        if p_type == 'SES':
            ...
            hilb_env_dc = hilb_env - np.mean(hilb_env)  # remove DC from envelope
            ses = np.abs(scifft(hilb_env_dc, nfft)) / len(signal)
            if p_spect > 1:
                ses = np.abs(scifft(hilb_env_dc, nfft))**p_spect / len(signal)
        elif p_type == 'ES':
        ...
        if p_type == 'SES':
            ...
            if p_spect > 1:
                ses = np.abs(scifft(env_dc, nfft))**p_spect/ len(signal)
        elif p_type == 'ES':
            env = np.abs(signal)
            env_dc = env - np.mean(env)  # remove DC from envelope
            ses = np.abs(scifft(env_dc, nfft)) / len(signal)
...
def analyse_ses(
    ...
    This reproduces the original implementation (used in the dissertation):
    ...
    for method in ['f', 't']:   # 'f' = rectification, 't' = Hilbert
        for case_idx in range(no_case):
            ...
            for nfft in nfft_values:
                ...
                p_full, _, _ = ses(signal, nfft, fs, bpfo_list, method, p_type, bw, p_spect, band=band)
                ...
                for i in range(num_segments):
                    ...
                    p_seg, _, _ = ses(segment, nfft, fs, bpfo_list, method, p_type, bw, p_spect, band=band)
